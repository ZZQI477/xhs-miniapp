<script>
	...
	import { resetSessionPromptFlag } from '@/utils/profileCheck.js'

	export default {
		onLaunch: function() {
			console.log('App Launch')
			// 重置会话提示标记，确保每次打开小程序时弹框状态重置
			resetSessionPromptFlag()
			// 检查登录状态
		...
		methods: {
			...
			async checkLoginStatus() {
				...
				// 小红书环境下检查 session 是否有效（仅用于日志，不决定业务 token 存亡）
				// session_key 有效期约2小时，真机冷启动常过期，与业务 token 无关
				if (isXhsMiniApp()) {
					const sessionValid = await checkSession()
					if (!sessionValid) {
						console.log('[App] 小红书 session 已过期，但业务 token 仍可能有效，继续验证')
					}
				}

				// 关键：调用后端接口验证业务 token 是否仍然有效，并刷新用户信息
				try {
