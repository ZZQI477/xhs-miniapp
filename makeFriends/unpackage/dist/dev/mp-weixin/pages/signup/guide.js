  ...
  methods: {
    ...
    async loadProvinces() {
      try {
        ...
        common_vendor.index.__f__("error", "at pages/signup/guide.vue:337", "加载省份失败", e);
    ...
    async loadCities(provinceId) {
      try {
        ...
        common_vendor.index.__f__("error", "at pages/signup/guide.vue:358", "加载城市失败", e);
    ...
    async submitForm() {
      ...
      try {
        ...
        try {
          ...
          common_vendor.index.__f__("log", "at pages/signup/guide.vue:549", "[Guide] 已更新本地用户信息", { gender: latestUserInfo.gender, education: latestUserInfo.education });
        } catch (err) {
          common_vendor.index.__f__("error", "at pages/signup/guide.vue:551", "[Guide] 更新本地用户信息失败", err);
        }
    ...
    async getCurrentLocation() {
      try {
        common_vendor.index.__f__("log", "at pages/signup/guide.vue:608", "开始获取位置...");
        const locationRes = await common_vendor.index.getLocation({
          type: "wgs84"
        });
        common_vendor.index.__f__("log", "at pages/signup/guide.vue:614", "获取位置成功:", locationRes);
        ...
        common_vendor.index.__f__("log", "at pages/signup/guide.vue:628", "逆地理编码响应:", geocodeRes);
        if (geocodeRes.data) {
          common_vendor.index.__f__("log", "at pages/signup/guide.vue:631", "逆地理编码数据:", geocodeRes.data);
          if (geocodeRes.data.status === "1") {
            if (geocodeRes.data.regeocode && geocodeRes.data.regeocode.addressComponent) {
              ...
              common_vendor.index.__f__("log", "at pages/signup/guide.vue:637", "当前位置:", provinceName, cityName);
              await this.$nextTick();
              common_vendor.index.__f__("log", "at pages/signup/guide.vue:641", "省份数据:", this.provinceOptions);
              if (provinceName && cityName) {
                await this.setLocationCity(provinceName, cityName);
              } else {
                common_vendor.index.__f__("error", "at pages/signup/guide.vue:647", "地址信息不完整:", addressComponent);
              }
            } else {
              common_vendor.index.__f__("error", "at pages/signup/guide.vue:650", "没有地址组件信息:", geocodeRes.data);
            }
          } else {
            common_vendor.index.__f__("error", "at pages/signup/guide.vue:653", "逆地理编码失败:", geocodeRes.data.info);
          }
        } else {
          common_vendor.index.__f__("error", "at pages/signup/guide.vue:656", "逆地理编码响应异常:", geocodeRes);
        }
      } catch (e) {
        common_vendor.index.__f__("error", "at pages/signup/guide.vue:659", "获取位置失败:", e);
      }
    },
    // 根据省份和城市名称设置城市选择器
    async setLocationCity(provinceName, cityName) {
      try {
        ...
        if (!province) {
          common_vendor.index.__f__("error", "at pages/signup/guide.vue:670", "未找到省份:", provinceName);
          return;
        }
        await this.loadCities(province.id);
        const city = this.cityOptions.find((c) => c.name === cityName);
        if (!city) {
          common_vendor.index.__f__("error", "at pages/signup/guide.vue:680", "未找到城市:", cityName);
          return;
        }
        this.form.provinceId = province.id;
        this.form.provinceName = province.name;
        ...
        this.cityValue = [provinceIndex, cityIndex];
        this.saveData();
        common_vendor.index.__f__("log", "at pages/signup/guide.vue:700", "自动定位成功:", provinceName, cityName);
      } catch (e) {
        common_vendor.index.__f__("error", "at pages/signup/guide.vue:702", "设置城市失败:", e);
      }
    }
  }
};
