  ...
  methods: {
    ...
    async loadBanners() {
      try {
        ...
        common_vendor.index.__f__("error", "at pages/single/index.vue:229", "加载Banner失败", e);
      }
    },
    // 加载用户列表
    async loadUsers() {
      ...
      try {
        ...
        common_vendor.index.__f__("error", "at pages/single/index.vue:268", "加载用户列表失败", e);
      } finally {
        this.loading = false;
      }
    },
    ...
    async loadCoupleList() {
      ...
      try {
        ...
        common_vendor.index.__f__("error", "at pages/single/index.vue:296", "加载脱单案例失败", e);
      } finally {
        this.loading = false;
      }
    },
    ...
    async handleVipPay(packageInfo) {
      ...
      try {
        ...
        common_vendor.index.requestPayment({
          ...
          fail: (err) => {
            common_vendor.index.__f__("error", "at pages/single/index.vue:448", "支付失败", err);
...
function _sfc_render(_ctx, _cache, $props, $setup, $data, $options) {
  return common_vendor.e({
    a: common_vendor.o(_ctx.onProfileGuide, "ed"),
    b: common_vendor.o(_ctx.onProfileDismiss, "81"),
    c: common_vendor.p({
      visible: _ctx.showProfileModal,
      percent: _ctx.profilePercent,
      ["missing-fields"]: _ctx.profileMissingFields
    }),
    d: common_vendor.o($options.onFilterConfirm, "03"),
    e: common_vendor.o(($event) => $data.showFilterModal = false, "d4"),
    f: common_vendor.p({
      visible: $data.showFilterModal,
      filter: $data.filterParams
    }),
    g: common_vendor.o(($event) => $data.showQrcodeModal = false, "31"),
    h: common_vendor.p({
      visible: $data.showQrcodeModal,
      qrcodeUrl: $data.groupQrcode,
      title: "告白时刻Daily公众号"
    }),
    i: common_vendor.o(($event) => $data.showVipPopup = false, "e4"),
    j: common_vendor.o($options.handleVipPay, "f3"),
    k: common_vendor.p({
    ...
    m: $data.banners.length > 0
  }, $data.banners.length > 0 ? {
    n: common_vendor.f($data.banners, (banner, index, i0) => {
      return {
        a: banner.image,
        b: index
      };
    ...
    o: $data.currentTab === 0 ? 1 : "",
    p: common_vendor.o(($event) => $options.switchTab(0), "c1"),
    q: $data.currentTab === 1 ? 1 : "",
    r: common_vendor.o(($event) => $options.switchTab(1), "0a"),
    s: $data.currentTab !== 2
  }, $data.currentTab !== 2 ? {
    t: common_assets._imports_0$2,
    v: common_vendor.o(($event) => $data.showFilterModal = true, "3f")
  } : {}, {
    w: $data.currentTab !== 2
  }, $data.currentTab !== 2 ? common_vendor.e({
    x: common_vendor.f($data.users, (user, index, i0) => {
      return common_vendor.e({
        a: user.avatar,
    ...
    z: $data.extraUsers.length > 0
  }, $data.extraUsers.length > 0 ? {
    A: common_vendor.f($data.extraUsers.slice(0, 5), (user, idx, i0) => {
      return {
    ...
    B: common_vendor.o((...args) => $options.handleUnlockClick && $options.handleUnlockClick(...args), "26")
  }) : {}) : {}, {
    C: $data.currentTab === 2
  }, $data.currentTab === 2 ? {
    D: common_vendor.f($data.coupleList, (item, index, i0) => {
      return {
        a: item.avatar,
    ...
    F: common_vendor.t($data.loading ? "加载中..." : "加载更多"),
    G: common_vendor.o((...args) => $options.loadMore && $options.loadMore(...args), "38")
  } : {}, {
    H: !$data.loading && $options.currentList.length === 0
...
//# sourceMappingURL=../../../.sourcemap/mp-weixin/pages/single/index.js.map
