...
const _sfc_main = {
  ...
  onShareAppMessage() {
    ...
    const shareConfig = {
      title: `告白时刻Daily`,
      // todo：分享图片写死图片
      imageUrl: utils_config.curlRef + "/uploads/sharecover.jpg",
      // imageUrl: '/static/images/share-cover.jpg'
      path: `/pages/user/detail?id=${this.currentUser.id}&share=true&inviter_id=${inviterId}`
    };
    common_vendor.index.__f__("log", "at pages/index/index.vue:313", "[ShareDebug] 首页分享:", shareConfig, "currentUser:", this.currentUser);
    if (this.currentUser) {
      return shareConfig;
    }
    return {
  ...
  methods: {
    ...
    async loadRecommendList() {
      ...
      try {
        ...
        common_vendor.index.__f__("error", "at pages/index/index.vue:360", "加载推荐列表失败", e);
      } finally {
        this.loading = false;
      }
    },
    ...
    async checkProfileComplete() {
      try {
        ...
        common_vendor.index.__f__("log", "at pages/index/index.vue:419", "用户资料:", userInfo);
        ...
        for (const field of requiredFields) {
          ...
          common_vendor.index.__f__("log", "at pages/index/index.vue:443", `${field.name}: ${value}`);
          if (value === null || value === void 0 || value === "") {
            missingFields.push(field.name);
          }
        }
        common_vendor.index.__f__("log", "at pages/index/index.vue:450", "缺少的字段:", missingFields);
        ...
        common_vendor.index.__f__("error", "at pages/index/index.vue:469", "检查资料失败:", error);
    ...
    handleServiceContact(e) {
      common_vendor.index.__f__("log", "at pages/index/index.vue:532", "客服消息事件", e);
    },
    // 订阅（开通VIP）
    handleSubscribe() {
...
function _sfc_render(_ctx, _cache, $props, $setup, $data, $options) {
  return common_vendor.e({
    ...
    j: $data.currentIndex,
    k: common_vendor.o((...args) => $options.onCardSwiperChange && $options.onCardSwiperChange(...args), "60"),
    l: common_vendor.o((...args) => $options.onCardSwiperAnimationFinish && $options.onCardSwiperAnimationFinish(...args), "f4")
  } : {}, {
    m: !$data.loading && $data.users.length === 0
  }, !$data.loading && $data.users.length === 0 ? {
    n: common_assets._imports_0$1,
    o: common_vendor.o((...args) => $options.loadRecommendList && $options.loadRecommendList(...args), "6c")
  } : {}, {
    p: $data.users.length > 0 && $options.currentUser
  }, $data.users.length > 0 && $options.currentUser ? {
    q: $options.currentUser.is_followed ? "/static/ygz.png" : "/static/Frame 1420074377.png",
    r: $options.currentUser.is_followed ? 1 : "",
    s: common_vendor.o((...args) => $options.handleFollow && $options.handleFollow(...args), "a0"),
    t: common_assets._imports_9,
    v: common_vendor.o((...args) => $options.handleContact && $options.handleContact(...args), "e4"),
    w: common_assets._imports_10,
    x: common_vendor.o((...args) => $options.handleServiceContact && $options.handleServiceContact(...args), "66")
  } : {}, {
    y: $data.users.length > 0
  }, $data.users.length > 0 ? {
