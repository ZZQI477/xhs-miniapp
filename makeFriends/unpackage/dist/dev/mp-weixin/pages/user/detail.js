...
const _sfc_main = {
  ...
  onShareAppMessage() {
    ...
    let shareImageUrl = utils_config.curlRef + "/uploads/sharecover.jpg";
  ...
  methods: {
    async loadUserDetail() {
      try {
        ...
        common_vendor.index.__f__("log", "at pages/user/detail.vue:532", "用户详情API返回:", res);
        if (res && res.data && res.data.user) {
          common_vendor.index.__f__("log", "at pages/user/detail.vue:534", "用户数据:", res.data.user);
          common_vendor.index.__f__("log", "at pages/user/detail.vue:535", "用户图片:", res.data.user.images);
          common_vendor.index.__f__("log", "at pages/user/detail.vue:536", "用户头像:", res.data.user.avatar);
          this.userInfo = this.transformUserData(res.data.user);
          common_vendor.index.__f__("log", "at pages/user/detail.vue:538", "转换后的userInfo:", this.userInfo);
          common_vendor.index.__f__("log", "at pages/user/detail.vue:539", "图片列表imgList:", this.userInfo.imgList);
        ...
        common_vendor.index.__f__("error", "at pages/user/detail.vue:548", "加载用户详情失败:", error);
        ...
        if (isUserNotFound) {
          ...
          common_vendor.index.__f__("log", "at pages/user/detail.vue:559", "[Detail] 用户不存在，切换为游客态浏览");
        } else if (isAuthError) {
          ...
          common_vendor.index.__f__("log", "at pages/user/detail.vue:567", "[Detail] 登录状态过期，已清除本地存储，降级为游客态");
      ...
      if (user.images) {
        if (Array.isArray(user.images) && user.images.length > 0) {
          ...
          try {
            ...
            common_vendor.index.__f__("error", "at pages/user/detail.vue:599", "解析图片JSON失败:", e);
          }
        }
      }
      common_vendor.index.__f__("log", "at pages/user/detail.vue:604", "处理后的图片列表:", imgList);
      return {
    ...
    handleImageError(e) {
      common_vendor.index.__f__("error", "at pages/user/detail.vue:678", "[ImageDebug] 图片加载失败:", e);
    },
    handleAvatarError(e) {
      common_vendor.index.__f__("error", "at pages/user/detail.vue:682", "[ImageDebug] 头像加载失败，清空avatar使用默认图标:", e);
      this.userInfo.avatar = "";
    },
    // 调试用：验证图片URL是否可访问
    debugCheckImage(url, label = "图片") {
      if (!url) {
        common_vendor.index.__f__("warn", "at pages/user/detail.vue:689", `[ImageDebug] ${label}: URL为空`);
        return;
      }
      common_vendor.index.__f__("log", "at pages/user/detail.vue:692", `[ImageDebug] ${label} URL:`, url);
      if (url.startsWith("/static/")) {
        common_vendor.index.__f__("log", "at pages/user/detail.vue:696", `[ImageDebug] ${label}: 本地图片，路径正确`);
        return;
      }
      common_vendor.index.getImageInfo({
        src: url,
        success: (res) => {
          common_vendor.index.__f__("log", "at pages/user/detail.vue:704", `[ImageDebug] ${label} 验证成功:`, res.width + "x" + res.height, res.type);
        },
        fail: (err) => {
          common_vendor.index.__f__("error", "at pages/user/detail.vue:707", `[ImageDebug] ${label} 验证失败:`, err.errMsg, "URL:", url);
        }
      });
    },
    // 检查是否已登录，未登录则弹出注册引导
    ...
    async wantSee(type) {
      ...
      try {
        ...
        common_vendor.index.__f__("error", "at pages/user/detail.vue:744", "想看操作失败:", error);
        common_vendor.index.showToast({
    ...
    async handleFollow() {
      ...
      try {
        ...
        common_vendor.index.__f__("error", "at pages/user/detail.vue:772", "关注操作失败:", error);
        common_vendor.index.showToast({
          title: error.msg || "操作失败",
          icon: "none"
        });
    ...
    handleShare() {
      common_vendor.index.__f__("log", "at pages/user/detail.vue:816", "分享");
    },
    ...
    async handleBind() {
      try {
        common_vendor.index.__f__("log", "at pages/user/detail.vue:835", "执行绑定");
      } catch (error) {
        common_vendor.index.__f__("error", "at pages/user/detail.vue:837", "绑定失败:", error);
      }
    },
    copyWechat(wechat) {
      if (wechat) {
    ...
    async checkProfileComplete() {
      try {
        ...
        common_vendor.index.__f__("log", "at pages/user/detail.vue:866", "用户资料:", userInfo);
        ...
        for (const field of requiredFields) {
          ...
          common_vendor.index.__f__("log", "at pages/user/detail.vue:890", `${field.name}: ${value}`);
          if (value === null || value === void 0 || value === "") {
            missingFields.push(field.name);
          }
        }
        common_vendor.index.__f__("log", "at pages/user/detail.vue:897", "缺少的字段:", missingFields);
        ...
        common_vendor.index.__f__("error", "at pages/user/detail.vue:918", "检查资料失败:", error);
        common_vendor.index.showToast({
          title: "检查资料失败",
          icon: "none"
        });
