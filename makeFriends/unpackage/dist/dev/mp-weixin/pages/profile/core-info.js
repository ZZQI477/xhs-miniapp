...
const _sfc_main = {
  ...
  methods: {
    ...
    initOptions() {
      ...
      const incomes = [0, 5, 10, 15, 20, 25, 30, 40, 50, 60, 70, 80, 90, 100, 200];
    ...
    async loadUserInfo() {
      try {
        ...
        common_vendor.index.__f__("log", "at pages/profile/core-info.vue:323", "[CoreInfo] 加载用户信息", userinfo);
        common_vendor.index.__f__("log", "at pages/profile/core-info.vue:326", "[CoreInfo] birthday:", userinfo.birthday);
        if (userinfo.birthday) {
          const year = new Date(userinfo.birthday).getFullYear();
          common_vendor.index.__f__("log", "at pages/profile/core-info.vue:329", "[CoreInfo] setting birthday:", year);
        ...
        common_vendor.index.__f__("log", "at pages/profile/core-info.vue:363", "[CoreInfo] income:", userinfo.income);
        if (userinfo.income !== void 0 && userinfo.income !== null && userinfo.income !== "") {
          const income = parseInt(userinfo.income) || 0;
          common_vendor.index.__f__("log", "at pages/profile/core-info.vue:366", "[CoreInfo] setting income:", income);
        ...
        common_vendor.index.__f__("log", "at pages/profile/core-info.vue:376", "[CoreInfo] 表单数据已填充", this.formData);
      } catch (e) {
        common_vendor.index.__f__("error", "at pages/profile/core-info.vue:379", "加载用户信息失败", e);
      }
    },
    // 加载地区名称
    async loadAreaNames(field, provinceCode, cityCode) {
      try {
        ...
        common_vendor.index.__f__("error", "at pages/profile/core-info.vue:404", "加载地区名称失败", e);
      }
    },
    // 加载地区数据
    async loadAreaData() {
      try {
        ...
        common_vendor.index.__f__("error", "at pages/profile/core-info.vue:419", "加载地区数据失败", e);
      }
    },
    // 加载城市数据
    async loadCities(provinceId) {
      ...
      try {
        ...
        common_vendor.index.__f__("error", "at pages/profile/core-info.vue:436", "加载城市数据失败", e);
      }
    },
    onBirthdayChange(e) {
    ...
    async uploadImage(filePath) {
      try {
        ...
        common_vendor.index.__f__("error", "at pages/profile/core-info.vue:556", "[CoreInfo] 上传图片失败", e);
    ...
    async handleSubmit() {
      ...
      try {
        ...
        common_vendor.index.__f__("log", "at pages/profile/core-info.vue:614", "[CoreInfo] 提交数据", submitData);
        ...
        common_vendor.index.__f__("log", "at pages/profile/core-info.vue:639", "[CoreInfo] 本地存储已更新", userinfo);
        common_vendor.index.showToast({ title: "提交成功", icon: "success" });
        setTimeout(() => {
          common_vendor.index.navigateBack();
        }, 1500);
      } catch (e) {
        common_vendor.index.__f__("error", "at pages/profile/core-info.vue:648", "[CoreInfo] 提交失败", e);
...
function _sfc_render(_ctx, _cache, $props, $setup, $data, $options) {
  return common_vendor.e({
    ...
    A: common_vendor.t($data.formData.income ? $data.formData.income + "万" : "请选择"),
    ...
    H: common_vendor.o((...args) => $options.onHasHouseChange && $options.onHasHouseChange(...args), "28"),
    I: common_vendor.t($data.formData.name || "请填写"),
    J: common_vendor.o(($event) => $options.showPicker("name"), "5d"),
    K: common_vendor.t($data.contactFieldName),
    L: common_vendor.t($data.formData.wechatNo || "请填写"),
    M: common_vendor.o(($event) => $options.showPicker("wechatNo"), "b4"),
    N: $data.formData.avatar
  }, $data.formData.avatar ? {
    O: $data.formData.avatar
  } : {}, {
    P: common_vendor.o((...args) => $options.chooseImage && $options.chooseImage(...args), "04"),
    Q: common_vendor.o((...args) => $options.handleSubmit && $options.handleSubmit(...args), "b6"),
    R: $data.loading,
    S: common_vendor.o((...args) => $options.closePopup && $options.closePopup(...args), "e0"),
    T: common_vendor.t($options.pickerTitle),
    U: $data.currentField === "position" || $data.currentField === "name" || $data.currentField === "wechatNo",
    V: common_vendor.o((...args) => $options.confirmPicker && $options.confirmPicker(...args), "e3"),
    W: $options.inputPlaceholder,
    X: $data.currentField === "wechatNo" ? 50 : 20,
    Y: $data.inputValue,
    Z: common_vendor.o(($event) => $data.inputValue = $event.detail.value, "e9"),
    aa: $data.currentField === "position" || $data.currentField === "name" || $data.currentField === "wechatNo",
    ab: common_vendor.o(() => {
    }, "e2"),
    ac: $data.showPopup,
    ad: common_vendor.o((...args) => $options.closePopup && $options.closePopup(...args), "39")
  });
}
