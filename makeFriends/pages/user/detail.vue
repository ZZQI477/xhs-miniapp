      ...
      >
        <view class="content-wrapper">
		  ...
		  <view class="section">
		    ...
		    <view class="detail-grid">
		      ...
		      <view class="detail-item" v-if="userInfo.hometown">
		        ...
		        <text class="detail-value">{{ userInfo.hometown }}</text>
		      </view> 
          ...
          <view v-if="!isAdSource" class="tags-card card">
            ...
            <view v-if="userInfo.tags && userInfo.tags.length" class="tags-wrapper">
              <text v-for="(tag, index) in userInfo.tags" :key="index" class="tag-item-primary">
                {{ tag }}
              </text>
            </view>
          ...
          <view class="ideal-partner-card card">
            ...
            <view v-if="hasIdealPartner">
              ...
              <view v-if="userInfo.idealPartner.tags && userInfo.idealPartner.tags.length" class="tags-wrapper">
                <text v-for="(tag, index) in userInfo.idealPartner.tags" :key="index" class="tag-item-primary">
                  {{ tag }}
                </text>
              </view>
            </view>
...
<script>
import { getUserDetail, toggleFollow, sendRequest, wantView } from '@/api/index.js'
...
export default {
  ...
  computed: {
    ...
    hasIdealPartner() {
      return !!(
        this.userInfo.idealPartner &&
        (this.userInfo.idealPartner.introduce ||
         (this.userInfo.idealPartner.tags && this.userInfo.idealPartner.tags.length > 0))
      )
    }
  },
  onLoad(options) {
  ...
  onShareAppMessage() {
    ...
    // 分享卡片图片：优先用照片墙第一张，其次头像，最后固定封面图
	// todo: 分享的图片先临时用服务器上照片
    // let shareImageUrl = '/static/images/share-cover.jpg'
    let shareImageUrl = utils_config.curlRef + '/uploads/sharecover.jpg'
    // if (this.userInfo.imgList && this.userInfo.imgList.length > 0) {
    //   shareImageUrl = this.userInfo.imgList[0]
    // } else if (this.userInfo.avatar) {
    //   shareImageUrl = this.userInfo.avatar
    // }
