<template>
  <view class="guide-container">
    <!-- 初始提示页面 -->
    <view v-if="showStartGuide" class="start-guide-overlay">
      <!-- 背景图片 -->
      <image src="/static/bg1.png" mode="aspectFill" class="start-guide-bg"></image>
      <view class="start-guide-content">
        <view class="start-guide-icon">
          <image src="/static/logo.png" mode="aspectFit" class="logo-img"></image>
        </view>
        <view class="start-guide-title">告白时刻Daily</view>
        <view class="start-guide-text">
          真实有效的填写资料才能更好的给您做匹配，是否准备开始您的脱单之旅？
        </view>
        <button class="start-guide-btn" @click="handleStartGuide">我已知晓</button>
      </view>
    </view>
    ...
    <!-- 步骤指示器 -->
    <view class="steps-indicator" v-if="!showStartGuide">
      <view
    ...
    <!-- 底部按钮区域 -->
    <view class="bottom-actions" v-if="!showStartGuide">
    ...
    <!-- 位置权限弹窗 -->
    <view v-if="showLocationPopup && !showStartGuide" class="popup-overlay" @click="closeLocationPopup">
...
export default {
  data() {
    return {
      ...
      showStartGuide: true, // 是否显示初始提示页面
