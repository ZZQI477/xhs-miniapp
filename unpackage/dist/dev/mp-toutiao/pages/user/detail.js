...
const _sfc_main = {
  ...
  methods: {
    ...
    handleShare() {
      common_vendor.index.__f__("log", "at pages/user/detail.vue:906", "分享");
    },
    ...
    async handleGetUserInfoForGuest(e) {
      try {
        if (e.detail.errMsg !== "getUserInfo:ok") {
          common_vendor.index.__f__("warn", "at pages/user/detail.vue:927", "[GuestChat] 用户拒绝授权用户信息，使用匿名模式");
        ...
        common_vendor.index.__f__("log", "at pages/user/detail.vue:938", "[GuestChat] 游客身份创建成功", guestInfo);
        ...
        if (userInfo) {
          ...
          common_vendor.index.__f__("log", "at pages/user/detail.vue:948", "[GuestChat] 游客用户信息已更新", userInfo);
        }
        ...
        common_vendor.index.hideLoading();
        common_vendor.index.__f__("error", "at pages/user/detail.vue:960", "[GuestChat] 创建游客身份失败", e2);
        common_vendor.index.showToast({
          title: "请稍后重试",
          icon: "none"
        });
    ...
    async createGuestAndNavigate() {
      try {
        const inviterId = common_vendor.index.getStorageSync("share_inviter_id") || "";
        const guestInfo = await utils_guestAuth.getOrCreateGuestId(inviterId ? { inviter_id: inviterId } : {});
        common_vendor.index.__f__("log", "at pages/user/detail.vue:974", "[GuestChat] 游客身份创建成功", guestInfo);
        this.showShareRegisterPopup = false;
        common_vendor.index.navigateTo({
          url: `/pages/chat/detail?to_user_id=${this.userId}&guest=1`
        });
      } catch (e) {
        common_vendor.index.__f__("error", "at pages/user/detail.vue:982", "[GuestChat] 创建游客身份失败", e);
        common_vendor.index.showToast({
          title: "请稍后重试",
          icon: "none"
        });
    ...
    async handleBind() {
      try {
        common_vendor.index.__f__("log", "at pages/user/detail.vue:998", "执行绑定");
      } catch (error) {
        common_vendor.index.__f__("error", "at pages/user/detail.vue:1000", "绑定失败:", error);
      }
    },
    copyWechat(wechat) {
      if (wechat) {
    ...
    async checkProfileComplete() {
      try {
        ...
        const userInfo = common_vendor.index.getStorageSync("userinfo") || common_vendor.index.getStorageSync("userInfo") || {};
        common_vendor.index.__f__("log", "at pages/user/detail.vue:1029", "用户资料:", userInfo);
        ...
        for (const field of requiredFields) {
          ...
          common_vendor.index.__f__("log", "at pages/user/detail.vue:1068", `${field.name}: ${value}`);
          if (value === null || value === void 0 || value === "") {
            missingFields.push(field.name);
          }
        }
        common_vendor.index.__f__("log", "at pages/user/detail.vue:1075", "缺少的字段:", missingFields);
...
资料越真实完整，对方通过你申请的概率越高哦～`;
        ...
        common_vendor.index.__f__("error", "at pages/user/detail.vue:1094", "检查资料失败:", error);
        common_vendor.index.showToast({
          title: "检查资料失败",
          icon: "none"
        });
...
function _sfc_render(_ctx, _cache, $props, $setup, $data, $options) {
  return common_vendor.e({
    ...
    g: common_vendor.o((...args) => $options.gotoSingle && $options.gotoSingle(...args), "a3")
  } : $data.userInfo.isLogout ? {
    i: common_vendor.o((...args) => $options.gotoSingle && $options.gotoSingle(...args), "c3")
  } : common_vendor.e({
    ...
    k: common_vendor.f($data.userInfo.imgList, (img, index, i0) => {
      return {
        a: img,
        b: common_vendor.o(($event) => $options.previewImage(index), "b2"),
        c: index
      };
    }),
    l: common_vendor.o((...args) => $options.handleImageError && $options.handleImageError(...args), "0e")
  } : common_vendor.e({
    m: $data.userInfo.avatar
  }, $data.userInfo.avatar ? {
    n: $data.userInfo.avatar,
    o: common_vendor.o((...args) => $options.handleAvatarError && $options.handleAvatarError(...args), "36")
  } : {
    ...
    r: common_vendor.o(($event) => $options.wantSee("photo"), "5b")
  }), {
    ...
    aa: common_vendor.o(($event) => $options.wantSee("introduce"), "09")
  } : {}, {
    Y: !$data.isAdSource,
    ab: !$data.isAdSource
    ...
    ah: common_vendor.o(($event) => $options.wantSee("tag"), "95")
  }) : {}, {
    ...
    ay: common_vendor.o(($event) => $options.wantSee("idealPartner"), "ec")
  } : {}, {
    aw: !$data.isAdSource,
    ...
    aC: common_vendor.o((...args) => $options.gotoReport && $options.gotoReport(...args), "44")
  } : {}, {
    ...
    aN: common_vendor.o(($event) => $options.copyWechat($data.userInfo.privacyInfo.wechatNo), "9c")
  } : {}) : {}) : {}, {
    ...
    aP: common_vendor.f($data.recommendList, (item, index, i0) => {
      return {
        ...
        f: index,
        g: common_vendor.o(($event) => $options.gotoUserDetail(item), "00")
      };
    })
  } : {}, {
    ...
    aT: common_vendor.o((...args) => $options.handleShare && $options.handleShare(...args), "bb"),
    aU: common_assets._imports_13,
    aV: common_vendor.o((...args) => $options.handleGetWechat && $options.handleGetWechat(...args), "17")
  } : {}, {
    aW: $data.isBindMode
  }, $data.isBindMode ? {
    aX: common_vendor.o((...args) => $options.handleBind && $options.handleBind(...args), "33")
  } : {}, {
    aY: !$data.isBindMode
  }, !$data.isBindMode ? {
    aZ: $data.userInfo.isFollow ? "/static/ygz.png" : "/static/Frame 1420074377.png",
    ba: common_vendor.o((...args) => $options.handleFollow && $options.handleFollow(...args), "71")
  } : {}, {
    bb: common_vendor.o($options.onCenterModalConfirm, "8d"),
    bc: common_vendor.o($options.onCenterModalCancel, "80"),
    bd: common_vendor.p({
