...
function generateDeviceId() {
  try {
    ...
    common_vendor.index.__f__("warn", "at utils/guestAuth.js:25", "[guestAuth] 获取设备信息失败", e);
  }
...
function getGuestId() {
  ...
  if (guestExpTime && Date.now() > guestExpTime) {
    common_vendor.index.__f__("log", "at utils/guestAuth.js:54", "[guestAuth] 游客身份已过期，清除本地存储");
    clearGuestInfo();
    return null;
  }
  return guestId;
...
async function getOrCreateGuestId(params = {}) {
  ...
  if (existingGuestId && existingGuestToken && isGuestValid()) {
    common_vendor.index.__f__("log", "at utils/guestAuth.js:127", "[guestAuth] 使用本地已有的游客身份", existingGuestId);
    return {
  ...
  try {
    common_vendor.index.__f__("log", "at utils/guestAuth.js:137", "[guestAuth] 创建新的游客身份", params);
    ...
    if (res && res.data) {
      ...
      common_vendor.index.__f__("log", "at utils/guestAuth.js:158", "[guestAuth] 游客身份创建成功", guestInfo.guest_id);
      return {
    ...
    common_vendor.index.__f__("error", "at utils/guestAuth.js:167", "[guestAuth] 创建游客身份失败", e);
...
async function updateGuestUserInfo(userInfo) {
  ...
  const guestToken = getGuestToken();
  if (!guestId) {
    common_vendor.index.__f__("warn", "at utils/guestAuth.js:275", "[guestAuth] 没有游客身份，无法更新用户信息");
  ...
  try {
    ...
    if (guestToken) {
      common_vendor.index.__f__("log", "at utils/guestAuth.js:292", "[guestAuth] 同步游客用户信息到服务器", guestId, mergedInfo);
      ...
      if (res && res.data) {
        common_vendor.index.__f__("log", "at utils/guestAuth.js:304", "[guestAuth] 游客用户信息同步成功");
        return { success: true, userinfo: mergedInfo };
      }
    }
    return { success: true, userinfo: mergedInfo };
  } catch (e) {
    common_vendor.index.__f__("error", "at utils/guestAuth.js:311", "[guestAuth] 更新游客用户信息失败", e);
    return {
      success: true,
