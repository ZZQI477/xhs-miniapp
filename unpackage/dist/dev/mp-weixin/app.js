...
const _sfc_main = {
  ...
  methods: {
    ...
    async checkLoginStatus() {
      ...
      if (!token) {
        common_vendor.index.__f__("log", "at App.vue:23", "[App] 未登录，游客态浏览");
        common_vendor.index.setStorageSync("is_guest_mode", true);
        return;
      }
      const userinfo = common_vendor.index.getStorageSync("userinfo");
      if (!userinfo || !userinfo.id) {
        common_vendor.index.__f__("log", "at App.vue:33", "[App] 本地用户信息异常，清除后降级为游客态");
      ...
      try {
        ...
        if (!freshUserInfo.id) {
          common_vendor.index.__f__("log", "at App.vue:52", "[App] 后端返回用户信息异常，降级为游客态");
          this.clearLoginState();
          return;
        }
        common_vendor.index.setStorageSync("userinfo", freshUserInfo);
        common_vendor.index.__f__("log", "at App.vue:58", "[App] token 验证通过，用户信息已刷新");
        ...
        if (!hasGender || !hasEducation) {
          common_vendor.index.__f__("log", "at App.vue:64", "[App] 基础信息未完成，需要填写引导信息", { gender: freshUserInfo.gender, education: freshUserInfo.education });
          setTimeout(() => {
        ...
        if (e._isAuthError) {
          common_vendor.index.__f__("log", "at App.vue:77", "[App] token 已过期，降级为游客态");
          common_vendor.index.setStorageSync("is_guest_mode", true);
        } else {
          common_vendor.index.__f__("error", "at App.vue:81", "[App] 验证登录状态失败", e);
