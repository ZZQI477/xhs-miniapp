  ...
  methods: {
    ...
    async loadUserInfo() {
      try {
        ...
        common_vendor.index.__f__("log", "at pages/profile/core-info.vue:327", "[CoreInfo] 加载用户信息", userinfo);
        common_vendor.index.__f__("log", "at pages/profile/core-info.vue:330", "[CoreInfo] birthday:", userinfo.birthday);
        if (userinfo.birthday) {
          const year = new Date(userinfo.birthday).getFullYear();
          common_vendor.index.__f__("log", "at pages/profile/core-info.vue:333", "[CoreInfo] setting birthday:", year);
        ...
        common_vendor.index.__f__("log", "at pages/profile/core-info.vue:367", "[CoreInfo] income:", userinfo.income);
        if (userinfo.income !== void 0 && userinfo.income !== null && userinfo.income !== "") {
          const income = userinfo.income || 0;
          common_vendor.index.__f__("log", "at pages/profile/core-info.vue:371", "[CoreInfo] setting income:", income);
        ...
        common_vendor.index.__f__("log", "at pages/profile/core-info.vue:381", "[CoreInfo] 表单数据已填充", this.formData);
      } catch (e) {
        common_vendor.index.__f__("error", "at pages/profile/core-info.vue:384", "加载用户信息失败", e);
      }
    },
    // 加载地区名称
    async loadAreaNames(field, provinceCode, cityCode) {
      try {
        ...
        common_vendor.index.__f__("error", "at pages/profile/core-info.vue:409", "加载地区名称失败", e);
      }
    },
    // 加载地区数据
    async loadAreaData() {
      try {
        ...
        common_vendor.index.__f__("error", "at pages/profile/core-info.vue:424", "加载地区数据失败", e);
      }
    },
    // 加载城市数据
    async loadCities(provinceId) {
      ...
      try {
        ...
        common_vendor.index.__f__("error", "at pages/profile/core-info.vue:441", "加载城市数据失败", e);
      }
    },
    onBirthdayChange(e) {
    ...
    async uploadImage(filePath) {
      try {
        ...
        common_vendor.index.__f__("error", "at pages/profile/core-info.vue:561", "[CoreInfo] 上传图片失败", e);
    ...
    async handleSubmit() {
      ...
      try {
        ...
        common_vendor.index.__f__("log", "at pages/profile/core-info.vue:619", "[CoreInfo] 提交数据", submitData);
        ...
        common_vendor.index.__f__("log", "at pages/profile/core-info.vue:644", "[CoreInfo] 本地存储已更新", userinfo);
        common_vendor.index.showToast({ title: "提交成功", icon: "success" });
        setTimeout(() => {
          common_vendor.index.navigateBack();
        }, 1500);
      } catch (e) {
        common_vendor.index.__f__("error", "at pages/profile/core-info.vue:653", "[CoreInfo] 提交失败", e);
...
function _sfc_render(_ctx, _cache, $props, $setup, $data, $options) {
  return common_vendor.e({
    ...
    e: common_vendor.o((...args) => $options.onBirthdayChange && $options.onBirthdayChange(...args), "e3"),
    f: common_vendor.t($data.formData.maritalStatus || "请选择"),
    g: $options.maritalStatusRange,
    h: $options.maritalStatusIndex,
    i: common_vendor.o((...args) => $options.onMaritalStatusChange && $options.onMaritalStatusChange(...args), "e8"),
    j: common_vendor.t($data.formData.livingPlaceText || "请选择"),
    k: $options.livingPlaceRange,
    l: $data.livingPlaceValue,
    m: common_vendor.o((...args) => $options.onLivingPlaceChange && $options.onLivingPlaceChange(...args), "a6"),
    n: common_vendor.o((...args) => $options.onLivingPlaceColumnChange && $options.onLivingPlaceColumnChange(...args), "89"),
    o: common_vendor.t($data.formData.hometownText || "请选择"),
    p: $options.hometownRange,
    q: $data.hometownValue,
    r: common_vendor.o((...args) => $options.onHometownChange && $options.onHometownChange(...args), "fc"),
    s: common_vendor.o((...args) => $options.onHometownColumnChange && $options.onHometownColumnChange(...args), "6e"),
    t: common_vendor.t($data.formData.education || "请选择"),
    v: $options.educationRange,
    w: $options.educationIndex,
    x: common_vendor.o((...args) => $options.onEducationChange && $options.onEducationChange(...args), "5b"),
    y: common_vendor.t($data.formData.position || "请填写"),
    z: common_vendor.o(($event) => $options.showPicker("position"), "15"),
    A: common_vendor.t($data.formData.income ? $data.formData.income : "请选择"),
    B: $options.incomeRange,
    C: $options.incomeIndex || void 0,
    D: common_vendor.o((...args) => $options.onIncomeChange && $options.onIncomeChange(...args), "04"),
    E: common_vendor.t($data.formData.hasHouse || "请选择"),
    F: $options.hasHouseRange,
    G: $options.hasHouseIndex,
    H: common_vendor.o((...args) => $options.onHasHouseChange && $options.onHasHouseChange(...args), "67"),
    I: common_vendor.t($data.formData.name || "请填写"),
    J: common_vendor.o(($event) => $options.showPicker("name"), "8c"),
    K: common_vendor.t($data.contactFieldName),
    L: common_vendor.t($data.formData.wechatNo || "请填写"),
    M: common_vendor.o(($event) => $options.showPicker("wechatNo"), "7e"),
    N: $data.formData.avatar
  }, $data.formData.avatar ? {
    O: $data.formData.avatar
  } : {}, {
    P: common_vendor.o((...args) => $options.chooseImage && $options.chooseImage(...args), "55"),
    Q: common_vendor.o((...args) => $options.handleSubmit && $options.handleSubmit(...args), "6c"),
    R: $data.loading,
    S: common_vendor.o((...args) => $options.closePopup && $options.closePopup(...args), "42"),
    T: common_vendor.t($options.pickerTitle),
    U: $data.currentField === "position" || $data.currentField === "name" || $data.currentField === "wechatNo",
    V: common_vendor.o((...args) => $options.confirmPicker && $options.confirmPicker(...args), "c6"),
    W: $options.inputPlaceholder,
    X: $data.currentField === "wechatNo" ? 50 : 20,
    Y: $data.inputValue,
    Z: common_vendor.o(($event) => $data.inputValue = $event.detail.value, "8e"),
    aa: $data.currentField === "position" || $data.currentField === "name" || $data.currentField === "wechatNo",
    ab: common_vendor.o(() => {
    }, "25"),
    ac: $data.showPopup,
    ad: common_vendor.o((...args) => $options.closePopup && $options.closePopup(...args), "41")
  });
}
const MiniProgramPage = /* @__PURE__ */ common_vendor._export_sfc(_sfc_main, [["render", _sfc_render], ["__scopeId", "data-v-ee97244e"]]);
wx.createPage(MiniProgramPage);
//# sourceMappingURL=../../../.sourcemap/mp-weixin/pages/profile/core-info.js.map
